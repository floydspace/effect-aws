import { DsqlSigner, DsqlSignerConfig } from "@effect-aws/dsql-signer";
import { Effect, Exit, Layer } from "effect";
import { pipe } from "effect/Function";
import { describe, expect, it } from "vitest";

describe("DsqlSigner", () => {
  it("defaultLayer", async () => {
    const layer = DsqlSigner.defaultLayer.pipe(
      Layer.provide(DsqlSignerConfig.layer({
        hostname: "localhost",
        region: "us-east-1",
        credentials: {
