 ...
 */
const currentEKSServiceConfig = globalValue(
  "@effect-aws/client-eks/currentEKSServiceConfig",
  () => FiberRef.unsafeMake<EKSService.Config>({}),
);
...
export const withEKSServiceConfig: {
  ...
  2,
  <A, E, R>(effect: Effect.Effect<A, E, R>, config: EKSService.Config): Effect.Effect<A, E, R> =>
    Effect.locally(effect, currentEKSServiceConfig, config),
);
 ...
 * @category eks service config
 */
export const setEKSServiceConfig = (config: EKSService.Config) => Layer.locallyScoped(currentEKSServiceConfig, config);
...
export const toEKSClientConfig: Effect.Effect<EKSClientConfig> = Effect.gen(function*() {
  const { logger: serviceLogger, ...config } = yield* FiberRef.get(currentEKSServiceConfig);
