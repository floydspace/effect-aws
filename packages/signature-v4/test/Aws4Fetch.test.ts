...
it.layer(credsLayer)(({ effect }) => 
  effect(`Appends AWS v4 signature headers to the request`, () => Effect.gen(function* () {
    ...
    expect(req.headers[`authorization`]).toEqual(
      expect.stringMatching(`AWS4-HMAC-SHA256 Credential=AKIAIOSFODNN7EXAMPLE/\\d{8}/eu-west-1/execute-api/aws4_request, SignedHeaders=accept;content-length;content-type;host;x-amz-date;x-amz-security-token, Signature=\\w+`)
    )
  }))
)
